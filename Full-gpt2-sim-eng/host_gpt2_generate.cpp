...
LayerWeights make_layer_weights(xrt::device &device, xrt::kernel &k0,
  ...
  return LayerWeights{
      make_bo(device, k0, 10, kHidden * F16, prefix + "c0_ln1_w"),
      make_bo(device, k0, 11, kHidden * F16, prefix + "c0_ln1_b"),
      make_bo(device, k0, 12, kHidden * 2304 * F16, prefix + "c0_qkv_w"),
      make_bo(device, k0, 13, 2304 * F16, prefix + "c0_qkv_b"),
      make_bo(device, k0, 14, kHidden * kHidden * F16, prefix + "c0_attn_out_w"),
      make_bo(device, k1, 4, kHidden * F16, prefix + "c1_attn_out_b"),
      make_bo(device, k1, 5, kHidden * F16, prefix + "c1_ln2_w"),
      make_bo(device, k1, 6, kHidden * F16, prefix + "c1_ln2_b"),
      make_bo(device, k1, 7, kHidden * 3072 * F16, prefix + "c1_fc1_w"),
      make_bo(device, k1, 8, 3072 * F16, prefix + "c1_fc1_b"),
      make_bo(device, k2, 3, 3072 * kHidden * F16, prefix + "c2_fc2_w"),
      make_bo(device, k2, 4, kHidden * F16, prefix + "c2_fc2_b"),
  };
}
...
void allocate_layer_caches(xrt::device &device, xrt::kernel &k0,
  ...
  for (int layer = 0; layer < kNumLayers; ++layer) {
    ...
    LayerCaches caches{
        make_bo(device, k0, 2, cache_bytes, prefix + "key_a"),
        make_bo(device, k0, 2, cache_bytes, prefix + "key_b"),
        make_bo(device, k0, 3, cache_bytes, prefix + "value_a"),
        make_bo(device, k0, 3, cache_bytes, prefix + "value_b"),
        true,
    };
...
HiddenBuffers allocate_hidden_buffers(xrt::device &device, xrt::kernel &k0,
  ...
  return HiddenBuffers{
      make_bo(device, k0, 0, 1 * 1 * kHidden * F16, "hidden_a"),
      make_bo(device, k2, 2, 1 * 1 * kHidden * F16, "hidden_b"),
  };
}
...
void preload_attention_biases(const fs::path &tensor_dir, xrt::device &device,
  ...
  for (size_t pos = 0; pos < kSeq; ++pos) {
    ...
    auto bias = make_bo(device, k0, 5, 1 * 1 * 1 * kSeq * F16,
...
int main(int argc, char **argv) {
  try {
    ...
    std::vector<LayerCaches> layer_caches;
    allocate_layer_caches(device, k0, layer_caches);
    std::vector<NamedBo> attention_biases;
    ...
    for (int pos = 0; pos < total_positions; ++pos) {
      ...
      for (int layer = 0; layer < kNumLayers; ++layer) {
        ...
        set_b3c2_args(r2, *hidden_out, weights.c2_fc2_w, weights.c2_fc2_b);
        set_b3c1_args(r1, weights.c1_attn_out_b, weights.c1_ln2_w,
        ...
        set_b3c0_args(r0, *hidden_in, cache_position, key_cache_in,
                      ...
                      key_cache_out, value_cache_out, weights.c0_ln1_w,
                      weights.c0_ln1_b, weights.c0_qkv_w, weights.c0_qkv_b,
                      weights.c0_attn_out_w);
