...
typedef struct {
    ...
    XB3c2_uio_map maps[ MAX_UIO_MAPS ];
} XB3c2_uio_info;

/***************** Variable Definitions **************************************/
static XB3c2_uio_info uio_info;
...
static int uio_info_read_name(XB3c2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XB3c2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XB3c2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XB3c2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XB3c2_Initialize(XB3c2 *InstancePtr, const char* InstanceName) {
	XB3c2_uio_info *InfoPtr = &uio_info;
