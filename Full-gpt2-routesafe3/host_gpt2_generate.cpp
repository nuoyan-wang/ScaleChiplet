...
int main(int argc, char **argv) {
  try {
    ...
    auto hidden_in =
        make_bo(device, k0, 0, 1 * 1 * 48 * kWordBytes, "hidden_in");
    auto key_cache_in =
        make_bo(device, k1, 6, 1 * kHeads * kSeq * 4 * kWordBytes,
                "key_cache_in");
    auto value_cache_in = make_bo(device, k1, 7,
                                  1 * kHeads * kSeq * 4 * kWordBytes,
                                  "value_cache_in");
    auto causal_bias_packed =
        make_bo(device, k1, 8, 1 * 1 * kCausalSeq * 4 * kWordBytes,
                "causal_bias");
    auto attention_bias = make_bo(device, k1, 9, 1 * 1 * 1 * 8 * kWordBytes,
                                  "attention_bias");
    auto key_cache_out =
        make_bo(device, k1, 10, 1 * kHeads * kSeq * 4 * kWordBytes,
                "key_cache_out");
    auto value_cache_out = make_bo(device, k1, 11,
                                   1 * kHeads * kSeq * 4 * kWordBytes,
                                   "value_cache_out");
    auto c0_ln1_w = make_bo(device, k0, 3, 48 * kWordBytes, "c0_ln1_w");
    auto c0_ln1_b = make_bo(device, k0, 4, 48 * kWordBytes, "c0_ln1_b");
    auto c0_qkv_w =
        make_bo(device, k0, 5, kHidden * 144 * kWordBytes, "c0_qkv_w");
    auto c0_qkv_b = make_bo(device, k1, 12, 144 * kWordBytes, "c0_qkv_b");
    auto c0_attn_out_w =
        make_bo(device, k2, 4, kHidden * 48 * kWordBytes, "c0_attn_out_w");

    auto c1_attn_out_b =
        make_bo(device, k2, 5, 48 * kWordBytes, "c1_attn_out_b");
    auto c1_ln2_w = make_bo(device, k2, 6, 48 * kWordBytes, "c1_ln2_w");
    auto c1_ln2_b = make_bo(device, k2, 7, 48 * kWordBytes, "c1_ln2_b");
    auto c1_fc1_w =
        make_bo(device, k2, 8, kHidden * 192 * kWordBytes, "c1_fc1_w");
    auto c1_fc1_b = make_bo(device, k2, 9, 192 * kWordBytes, "c1_fc1_b");

    auto hidden_out =
        make_bo(device, k2, 3, 1 * 1 * 48 * kWordBytes, "hidden_out");
    auto c2_fc2_w =
        make_bo(device, k2, 10, 3072 * 48 * kWordBytes, "c2_fc2_w");
    auto c2_fc2_b = make_bo(device, k2, 11, 48 * kWordBytes, "c2_fc2_b");
    ...
    for (int pos = 0; pos < total_positions; ++pos) {
      ...
      for (int layer = 0; layer < kNumLayers; ++layer) {
        ...
        set_s3c2_args(r2, hidden_out, c0_attn_out_w, c1_attn_out_b,
                      c1_ln2_w, c1_ln2_b, c1_fc1_w, c1_fc1_b, c2_fc2_w,
                      c2_fc2_b);
        set_s3c1_args(r1, pos, key_cache_in, value_cache_in,
                      causal_bias_packed, attention_bias, key_cache_out,
                      value_cache_out, c0_qkv_b);
        set_s3c0_args(r0, hidden_in, c0_ln1_w, c0_ln1_b, c0_qkv_w);
